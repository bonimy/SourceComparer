// <copyright file="ByteFormatter.cs" company="Public Domain">
...
namespace nom.tam.util
{
    ...
    public sealed class ByteFormatter
    {
        ...
        /// <summary>Set the truncation behavior.</summary>
        /// <param name="val">If set to true (the default) then do not
        /// exceed the requested length.  If a number cannot
        /// be sensibly formatted, the truncation fill character
        /// may be inserted.</param>
        ...
        /// <summary>Set the truncation fill character.</summary>
        /// <param name="val">The character to be used in subsequent truncations.</param>
        ...
        /// <param name="val">Should numbers be right aligned?</param>
        public bool Align
        {
            set
            {
        ...
        /// locations.
        /// </summary>
        //		private static sbyte[] digits = new sbyte[]{(sbyte) SupportClass.Identity('0'), (sbyte) SupportClass.Identity('1'), (sbyte) SupportClass.Identity('2'), (sbyte) SupportClass.Identity('3'), (sbyte) SupportClass.Identity('4'), (sbyte) SupportClass.Identity('5'), (sbyte) SupportClass.Identity('6'), (sbyte) SupportClass.Identity('7'), (sbyte) SupportClass.Identity('8'), (sbyte) SupportClass.Identity('9')};
        ...
        public int format(int val, byte[] buf, int off, int len)
        {
            ...
            // Special case
                  ...
                  if (val < 0)
                  {
                      buf[xoff] = (sbyte) SupportClass.Identity('-');
                  }
        ...
        public int format(long val, byte[] buf, int off, int len)
        {
            ...
            /*
                  ...
                  // Special case
                        ...
                        buf[xoff] = (sbyte) SupportClass.Identity('0');
                        bool last = (pos == 0);
        ...
        public int format(float val, byte[] buf, int off, int len)
        {
            ...
            /*
                  ...
                  //UPGRADE_ISSUE: Method 'java.lang.Float.floatToIntBits' was not converted. 'ms-help://MS.VSCC.2003/commoner/redir/redirect.htm?keyword="jlca1000_javalangFloatfloatToIntBits_float"'
        ...
        public int format(double val, byte[] buf, int off, int len)
        {
            ...
            /*
                        ...
                        //UPGRADE_ISSUE: Method 'java.lang.Double.doubleToLongBits' was not converted. 'ms-help://MS.VSCC.2003/commoner/redir/redirect.htm?keyword="jlca1000_javalangDoubledoubleToLongBits_double"'
                        ...
                        long numb = (bits & (int) SupportClass.Identity(0x000FFFFFFFFFFFFFL));
        ...
        /*
		internal int combineReal(double val, sbyte[] buf, int off, int len, sbyte[] mant, int lmant, int shift)
		{
			...
			else
			{
				if (exp >= 0)
				{
					...
					{
						if (mant[i] != (sbyte) SupportClass.Identity('9'))
						{
							break;
						}
					}
					if (i > exp && i < lmant && mant[i] >= (sbyte) SupportClass.Identity('5'))
					{
						minSize += 1;
					}
			...
			if (val < 0)
			{
				buf[off] = (sbyte) SupportClass.Identity('-');
				off += 1;
				len -= 1;
			}
			...
			else
			{
				...
				buf[off] = (sbyte) SupportClass.Identity('E');
				off += 1;
		...
		*/
        ...
        /// </summary>
        /*
		internal int mantissa(sbyte[] mant, int lmant, int exp, bool simple, sbyte[] buf, int off, int len)
		{
			...
			if (exp < 0)
			{
				buf[off] = (sbyte) SupportClass.Identity('0');
				len -= 1;
				off += 1;
				if (len > 0)
				{
			...
			if (pos < lmant && mant[pos] >= (sbyte) SupportClass.Identity('5'))
			{
				int i;
				...
				{
					if (buf[i] == (sbyte) SupportClass.Identity('.') || buf[i] == (sbyte) SupportClass.Identity('-'))
					{
						continue;
					}
					if (buf[i] == (sbyte) SupportClass.Identity('9'))
					{
						buf[i] = (sbyte) SupportClass.Identity('0');
					}
					else
					{
						buf[i] = (sbyte) (buf[i] + 1);
				...
				if (i < off0)
				{
					buf[off0] = (sbyte) SupportClass.Identity('1');
					bool foundDecimal = false;
					for (i = off0 + 1; i < off; i += 1)
					{
						if (buf[i] == (sbyte) SupportClass.Identity('.'))
						{
							foundDecimal = true;
							if (simple)
							{
								...
								if (i < off)
								{
									buf[i] = (sbyte) SupportClass.Identity('.');
								}
							}
							break;
						}
					...
					{
						buf[off + 1] = (sbyte) SupportClass.Identity('0'); // 99 went to 100
						off += 1;
					}
        ...
        {
            {
                /*
                        ...
                        int min = (int)Math.Floor((int)(Math.Log(Double.MinValue) * ilog10));
                        ...
                        int max = (int)Math.Floor((int)(Math.Log(Double.MaxValue) * ilog10));
                        max += 1;
